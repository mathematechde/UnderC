...
protected:
 ...
 int m_mem_unit;
 int m_reserved;
 unsigned char m_pack_alignment;
 unsigned char m_max_alignment;
 int m_move_to_next;
public:
  Allocator(void *buff=NULL, int sz=0);
  ~Allocator();

  void set_pack_alignment(unsigned char alignment)
  { m_pack_alignment = alignment ? alignment : 1; }

  unsigned char pack_alignment() const { return m_pack_alignment; }
  VMByteSize aggregate_alignment() const { return m_max_alignment; }
  ...
  int size() const
  { return static_cast<int>(m_data - m_buff); }
  ...
  int alloc_aligned(int sz, int alignment, void *data);
  VMWord alloc_qword();

  int alloc_word(VMWord val)
  { return alloc(sizeof(VMWord),&val); }
  ...
  int offset(void *ptr)
  { return static_cast<int>(static_cast<char *>(ptr) - m_buff); }
