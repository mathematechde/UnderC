...
enum RMode { // address modes
  NONE,
  DIRECT,  // offset or pointer-sized payload in the data area
...
// UCRI instruction view. Pointer-bearing operands retain the host pointer
// width; this layout intentionally mirrors the maintained runtime record.
struct XInstruction {
  uint opcode;
  uint rmode;
  intptr_t data;
...
enum { XINSTRUCTION_INFO_VERSION = 1 };
struct XInstructionInfo {
#ifdef __UNDERC__
...
/// \param ptr a pointer to the converted value
	void    str_to_val(const char *buff, void *ptr);
	Type*   type();
/// convert an ASCII representation (e.g. "int") into a type.
	static XType*  from_str(const char *str);
...
public:
   ...
   /// convert a string to valid data for this entries' type.
    void str_to_val(const char *buff, void *base=NULL);
...
public:
 ...
 /// Get the function's pcode.
  ...
  /// \return A pointer to the pcode
    XInstruction* pcode();
  /// Number of instructions, excluding the terminal zero opcode.
    size_t instruction_count();
  /// Copy one instruction into a versioned public snapshot.
    int instruction_at(size_t index, XInstructionInfo *instruction);
  ...
  /// \sa XEntry
	XEntry* lookup_local(const char* name);
};
...
public:
  ...
  /// \sa XEntry
	virtual XEntry* lookup(const char *name,bool in_parent=false);
 /// Look up a class in this context
 /// \return A pointer to the class object, NULL if not found or not a class
 /// \sa XClass
	XClass* lookup_class(const char *name, bool in_parent=false);
 /// Look up a template in this context
	XTemplateFun* lookup_template(const char *name, bool in_p=false);
 ...
 /// \sa XEntry
    XEntry *create(const char *nm = "", XType *xt = NULL);
 ...
 /// \param pattern Can specify a wildcard
	void get_functions(XFunctions& fns, int flags = 0,const char *pattern=NULL);
 ...
 /// Again, \a flags can contain DO_PARENT as before.
 	void get_variables(XEntries& vars, int flags=FIELDS,const char *pattern=NULL);
