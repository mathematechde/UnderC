...
typedef std::list<VMWord> VMWordList;
...
public:
 ...
 void emit(int opcode, int rm=0, VMWord data=0);
 void emit_data_instruction(int opcode, VMWord data);
 void emit_push_int(VMWord sz); 
 void emit_reference(PExpr ex, int flags, int tcode, Type value_type);
