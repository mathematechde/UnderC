...
const int ARG = 101, TEMP = 103, DEFER_TEMP = 104;
...
struct ParserState {
  ...
  void finish_enum(bool has_definition);
...
struct DebugFlags {
   ...
   bool auto_exec;
   bool function_trace;
   VMByteSize class_pack_alignment;
   bool verbose;
   bool strict;
   bool ptr_check;
   ...
   int optimization_level;      // bytecode optimization level selected by -O0 through -O3
...
// Bison semantic values use an interned type handle.  They must not depend on
// the size or byte representation of Type.
typedef size_t TType;
Type AsType(TType handle);
TType AsTType(const Type& value);
void           tpush(Type t);
Type&          tots();
Type           tpop();
TType          ttots();
