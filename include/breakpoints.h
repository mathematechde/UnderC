...
public:
  ...
  static void toggle(string file, int lineno, bool is_persistent, ostream& out);
  static void group(string file, int *lines, int& sz, bool do_get);
  static iterator find_in_file(string file);
  static iterator   end_list();
  static Breakpoint *create(string filename, int lineno, bool persist); 
  static Breakpoint *from_id(int id);
  static Breakpoint *exists_at(string filename, int lineno);
