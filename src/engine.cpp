...
CEXPORT const char *sh_fun();  // debug function

// later in this file....
int current_ip();  
void  exec_message(const char *msg, bool was_error, bool dont_unwind = false);
...
void
CodeGenerator::emitc(int opc, RMode rm, VMWord data)
//------------------------------------------
{
 pcode->opcode = opc;
 pcode->rmode  = rm;
...
{
 if (NC) {
   optimize_bytecode(_code_buff_,NC,Parser::debug.optimization_level);
...
x_copy_code(Instruction *iptr)
{
 ...
 int sz = static_cast<int>(pi - iptr);
...
union VMStackStorage {
  double alignment;
  VMWord words[EXEC_STACKSIZE];
};
static VMStackStorage vm_stack_storage;
#define _stack_ vm_stack_storage.words
static VMWord *mSP = _stack_ + EXEC_STACKSIZE - 1;
static VMWord *mEndStack = _stack_;
const int DOUBLE_WORDS = (sizeof(double) + sizeof(VMWord) - 1) / sizeof(VMWord);
const int FLOAT_WORDS = (sizeof(float) + sizeof(VMWord) - 1) / sizeof(VMWord);
...
VMWord popp() { // specialized pointer-checking pop!
  VMWord p = pop();
  if (ptr_check) Builtin::bad_ptr_check(vm_to_ptr(p));
  return p;
}
...
inline void pushf(float v) {
     mSP -= FLOAT_WORDS; *((float *)mSP) = v;
}

inline float popf() { 
    float tmp = *((float *)mSP); mSP += FLOAT_WORDS; return tmp;
}

// quadword stack operations

inline void push2(double fval) {
    mSP -= DOUBLE_WORDS; *((double *)mSP) = fval; 
}

inline double pop2() {
	double tmp = *(double *)mSP; mSP += DOUBLE_WORDS; return tmp;  
}
...
int stack_depth() { return static_cast<int>(mSP - _stack_); }

void stack_report(ostream& os,VMWord *base)
{
    os << "STACK: ";
...
const Instruction *end_of_code =
  reinterpret_cast<Instruction *>(sizeof(Instruction));
...
#ifdef DEBUG_ODS
void destruct_error(const char *msg)
{
     PFunction fn;
     int ofs;
	 PClass pc;
...
#else
void destruct_error(const char *msg) { exec_message(msg,true); }
#endif
...
// -1 otherwise
{
 ...
 int n  = mODS.depth();
 if (!n) return -1;
 // index 0 is the oldest entry, so walk down from the top: an exception has to
 // be offered to the innermost enclosing try block first
 for(int i = n-1; i >= 0; i--)
 {
   PClass pc;
   void *obj;
   ...
   if (pc == Parser::try_block_class()) {
     ...
     curr_except = static_cast<CatchHandler *>(vm_to_ptr(*(VMWord *)obj));
...
//--------------- THE STACK ENGINE --------------------------
#define REF(T,p)    (*(T *)(p))
#define REFS(T)     (*(T *)vm_to_ptr(pop()))
#define REFA(T,p,a) (*((T *)(p)+(a)))
#define REFP(T,p)   (*(T **)(p))
#define NEXT(p)     ((VMWord *)p)+1


int switch_jump(VMWord *sb, VMWord val)
...
void start_function(FBlock *_fb)
{
 ...
 if (gFunBlock == fb)
  ...
  if (mSP < _stack_) throw Exception("Exec Stack overflow");
...
void get_stack_frame(int fi, ExecutionState& xs)
{
  if (fi == 0) {
    ...
    int offs = -3*(fi-1);
    xs.m_baseSP = (VMWord *)fs_ptr(offs);
...
VMWord *find_main_baseSP(char *fct)
{
   ExecutionState xs;
...
#ifdef _DEBUG
int sh_offs() {
   return static_cast<int>(mSP-_stack_);
}

CEXPORT const char *sh_fun() {
  static string tmp;
  try {
...
int current_ip()
{
  return static_cast<int>(ppcode - fb->pstart);
}

bool start_exec()
//---------------------------
{
  static int rmode,nargs;
  static VMWord rdata,val,tmp;
  static Opcodes opcode;
  static double fval, gval;
  static void *data;
  static VMWord buff[TMP_BUFF_SIZE];
...
top:
    ...
    case PEA:  push(vm_from_ptr(data)); break;
    ...
    case PUSHI: push(REF(VMInt,data)); break;
    ...
    case POPI:  REF(VMInt,data) = static_cast<VMInt>(pop()); break;
    case POPF:  REF(float,data) = (float)pop2();   break;
    case POPD:  REF(double,data) = pop2();  break;
    //....stack-relative pushes/pops
    case PUSHSC: push(REF(char,vm_to_ptr(popp()))); break;
    case PUSHSW: push(REF(short,vm_to_ptr(popp()))); break;
    case PUSHSI: push(REF(VMInt,vm_to_ptr(popp()))); break;
    case PUSHSF: push2(REF(float,vm_to_ptr(popp()))); break;
    case PUSHSD: push2(REF(double,vm_to_ptr(popp()))); break;
    case POPSC:  val=popp(); REF(char,vm_to_ptr(val)) = pop(); break;
    case POPSW:  val=popp(); REF(short,vm_to_ptr(val)) = pop(); break;
    case POPSI:  val=popp(); REF(VMInt,vm_to_ptr(val)) = static_cast<VMInt>(pop()); break;
    case POPSD:  val=popp(); REF(double,vm_to_ptr(val)) = pop2(); break;
    case PUSHP:  push(REF(VMWord,data)); break;
    case POPP:   REF(VMWord,data) = pop(); break;
    case PUSHSP: push(REF(VMWord,vm_to_ptr(popp()))); break;
    case POPSP:  val=popp(); REF(VMWord,vm_to_ptr(val)) = pop(); break;
    ...
    case IDIV:   val=pop();  reinterpret_cast<VMUWord&>(tos()) /= val;  break;
    ...
    //...array operations
    case ADDCC:  val=pop();  push(vm_from_ptr((char *)data + sizeof(char)*val)); break;
    case ADDCW:  val=pop();  push(vm_from_ptr((char *)data + sizeof(short)*val)); break;
    case ADDCI:  val=pop();  push(vm_from_ptr((char *)data + sizeof(VMInt)*val)); break;
    case ADDCD:  val=pop();  push(vm_from_ptr((char *)data + sizeof(double)*val)); break;
    case ADDPC:  val=pop();  push(REF(VMWord,data) + sizeof(char)*val); break;
    case ADDPW:  val=pop();  push(REF(VMWord,data) + sizeof(short)*val); break;
    case ADDPI:  val=pop();  push(REF(VMWord,data) + sizeof(VMInt)*val); break;
    case ADDPD:  val=pop();  push(REF(VMWord,data) + sizeof(double)*val); break;
    // ADDSC is ADD!
    case ADDSW:  val=pop();  push(val + sizeof(short)*pop());           break;  
    case ADDSI:  val=pop();  push(val + sizeof(VMInt)*pop()); break;
    case ADDSD:  val=pop();  push(val + sizeof(double)*pop());          break;
    case ADDSN:  val=pop();  { VMWord base=pop(); push(base + val*pop()); } break;

    case COPY:   { void *dst=vm_to_ptr(pop()); void *src=vm_to_ptr(pop()); memcpy(dst,src,ppcode->data); } break;
...
call_the_function:
    ...
    case CALLN:  // native code interface
        ...
        callfn(pfb,mSP-1,nargs,
               (pfb->flags & DC_METHOD) ? mOP : NULL,buff);
        mSP += nargs;
    ...
    case CCALLV: {// vector constructor/destructor call
        ...
        if (pcb->dynamic()) { // i.e new[] has generated a ptr for us; 
           // *fix 1.5.0 mOP is the object pointer that _new_vect_ex returned,
           // and that is the key the count was recorded under.  Backing up by
           // sizeof(void *) only found the block base while a VM word and an
           // int were the same width; on a 64-bit build it matched nothing and
           // alloc_size() fell back to 1, so exactly one element was built.
           num = Builtin::alloc_size(mOP);
        } else num = pcb->num();
        char *old_mOP = mOP;
        ...
        mOP = old_mOP;
        if (pcb->dynamic()) push(vm_from_ptr(mOP));  // leave obj on XS
        opop();                              // drop the OS
      }
      break;
  // *NOTE* The continue seems strange, but it is important to skip ppcode++!
    case CALLS:
        start_function(PFBlock(vm_to_ptr(pop())));
        continue;
    case VCALLS:
    ...
    case JSWITCH: ppcode = fb->pstart + switch_jump((VMWord *)data,pop()); goto top;
    case RET:   
          mSP = baseSP;
          mSP += fb->nargs;
          end_function();
    ...
    case DCAST: { // *add 0.9.5 dynamic cast
           // emit_type_instr() stores a Parser::TType handle, not a Type object
           Type t = Parser::AsType(*(Parser::TType *)data);
           Class *tc = t.as_class();  // hm...what about the (void *) possibility?
           void *ptr = vm_to_ptr(pop());
   ...
   // INCS and DECS
    ...
    case THROW_EX:   {
        Type et = Parser::AsType(*(Parser::TType *)data);
        void *obj = vm_to_ptr(pop());
    ...
    case CHKVMT: // *NOTE64*
      push(vm_from_ptr((*(PClass *)(data))->check_object_pointer((char *)vm_to_ptr(pop()))));
      break;
    ...
    case SHOWV:
    {
        ...
        PEntry ve;
        if (opcode==SHOWI) {
            t = Parser::AsType(*(Parser::TType *)data); ve = NULL;
            te = t;
        } else { 
...
void  exec_message(const char *msg, bool was_error, bool dont_unwind_error)
{
 if (fb == START_FB) {
