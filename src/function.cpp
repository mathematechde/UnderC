...
{
// *fix 1.5.0 Round every local up to a whole VM word before reserving it.
// The frame is addressed in words - set_mem_unit(sizeof(VMWord)) in the ctor
// above, and Table::alloc_aligned() returns the byte offset divided by that
// unit - but the cursor was advanced by the raw byte size.  A local that was
// not a whole number of words left the cursor mid-word, and the next
// variable's offset truncated back onto it: "int p; int q;" both landed on
// frame word 0, and "int a[3]; int i;" put i on top of a[0].  This only
// appeared on the 64-bit port, where an int is half a word; while VMWord was
// four bytes the division was always exact.  add_variable() already reserves
// vm_word_count(size) words for the offset it hands back, so rounding here
// just makes the cursor agree with the offsets already being issued.
 sz = vm_word_count(sz)*static_cast<int>(sizeof(VMWord));
...
void LocalContext::set_parent(Table *parent)
{
   ...
   // start off where our parent ended....
    if (parent->type()== FUNCTION) {
       if (m_data == 0)
         m_data = reinterpret_cast<char *>(static_cast<uintptr_t>(parent->size()));
    }
}
...
// set up native code call block;
 ...
 if (pfn->is_method() && !pfn->is_cdecl()) fblk.flags += DC_METHOD;
...
Function::Function(Signature *sig, FunctionEntry *pfe)
 : m_sig(sig),m_context(NULL),m_cdecl(false),m_slot_id(0),m_ret_obj(NULL),
   m_fun_entry(pfe),
   m_default_args(NULL),m_default_index(0),m_stdarg(false),m_builtin(false),
   m_ftype(0),
...
Function::~Function()
{
    Builtin::release_native_stubs(this);
    delete m_line_nos;
}

bool Function::is_destructor()  { return m_ftype==2; }

bool Function::is_constructor() { return m_ftype==1; }

bool Function::is_plain()       { return m_ftype==0; }

bool Function::is_method()      { return m_ftype!=0; }

FunctionContext * Function::context()     { return m_context; }

Signature *      Function::signature()   { return m_sig; }

Type              Function::return_type() { return m_sig->return_type(); }

LineNumbersB *   Function::line_nos()     { return m_line_nos; }

PEntry Function::return_object()   { return m_ret_obj; }

void Function::return_object(PEntry pe) { m_ret_obj = pe; }

bool Function::stdarg()            { return signature()->stdarg(); }

void Function::stdarg(bool stdarg) { signature()->stdarg(stdarg); }

bool Function::builtin()           { return m_builtin != 0;  }

void Function::builtin(uchar b)    { m_builtin = b;          }

bool Function::is_cdecl()          { return m_builtin == CDECL;  }

bool Function::stdmethod()                     { return m_builtin == STDMETHOD; }

ImportScheme *Function::import_scheme()        { return m_import_scheme; }

void Function::import_scheme(ImportScheme *is) { m_import_scheme = is; }

void Function::adjust_return_type(Type t) { m_sig->adjust_return_type(t); }

void Function::fun_block(FBlock *fblk) { m_fun_block = fblk; }

FBlock *Function::fun_block()          { return m_fun_block; }

FunctionEntry *Function::fun_entry()   { return m_fun_entry; }

bool Function::has_default_args()     { return m_default_args != NULL; }
...
Class *Function::class_context()    { return signature()->class_ptr(); }

bool Function::is_const()                  { return signature()->is_const(); }
bool Function::is_static()                 { return class_context() != NULL && !is_method(); } 
bool Function::export_as_cdecl()           { return m_cdecl; }
void Function::export_as_cdecl(bool yesno) { m_cdecl = yesno; }
int Function::slot_id()                    { return m_slot_id;  }
bool Function::is_virtual()                { return m_slot_id != 0; }
int  Function::line()                      { return m_line; }
int  Function::module()                    { return m_mod;  }

bool Function::is_template()                       { return m_templ_info != NULL; }
TemplateInstance *Function::get_template()         { return m_templ_info;  }
void Function::set_template(TemplateInstance *pti) { m_templ_info = pti; }
...
static char *string_to_buffer(string s)
{
 strcpy(buff,s.c_str());
 return buff;
}
...
char *sh_sig(Signature& s)
{
  ...
  *buff = 0;
  ...
  ostringstream outs;
  outs << s;
  return string_to_buffer(outs.str());
 #endif
}
