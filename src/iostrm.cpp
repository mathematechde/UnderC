...
top:
 if (fetch_line(m_buff,m_buffsize)) {
    ...
    m_s = empty_field; // to prevent conversions choking..
    return false;
 }
}
...
bool  ifs::fetch_line(char *buff, int n)
{
  ...
  m_line++;
  if (t == NULL) { if (n > 0) buff[0] = '\0'; return false; } // buff is untouched
...
iss& operator >> (iss& is, const char *s)
{
  strcpy((char*)s,is.gets());
  return is;
}
...
{
  char end_marker[] = "t";
  char *endptr = end_marker; // must not be NULL
...
iss& operator >> (iss& is, string s)
{
  strcpy((char*)s.c_str(),is.gets());
  return is;
}

// Insert into output stream....
oss& operator << (oss& os, const char *s)
{
 os.puts((char*)s);
 return os;
}
...
{
 snprintf(temp,sizeof(temp),os.m_radix == 16 ? "%x" : "%d",i);
 os.puts(temp);
 return os;
}
...
{
 snprintf(temp,sizeof(temp),os.m_radix == 16 ? "%lx" : "%ld",i);
 os.puts(temp);
 return os;
}
...
{
 os.puts(gcvt(f,os.m_digits,temp));
 return os;
}
...
{
 snprintf(temp,sizeof(temp),os.m_radix == 16 ? "%lx" : "%lu",i);
 os.puts(temp);
 return os;
}

// *fix wide VM words: the 64-bit ABI work made VMWord / VMFrameSlots /
// size_t-sized values reach these operators.  Without a 'long long' overload
// every such insertion is ambiguous between long / unsigned long / int.
oss& operator << (oss& os, long long i)
{
 char buff[32];
 snprintf(buff,sizeof(buff),os.m_radix == 16 ? "%llx" : "%lld",i);
 os.puts(buff);
 return os;
}

oss& operator << (oss& os, unsigned long long i)
{
 char buff[32];
 snprintf(buff,sizeof(buff),os.m_radix == 16 ? "%llx" : "%llu",i);
 os.puts(buff);
 return os;
}
...
oss& operator << (oss& os, string s)
{
 os.puts((char*)s.c_str());
 return os;
}
