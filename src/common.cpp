// @compile{g++ -c -D_CONSOLE common.cpp}
/* common.cpp
...
namespace {
  std::vector<Type> parser_types(1,Type());
}

Type Parser::AsType(Parser::TType handle)
{
  return handle < parser_types.size() ? parser_types[handle] : Type();
}

Parser::TType Parser::AsTType(const Type& value)
{
  for (size_t i = 0; i < parser_types.size(); ++i)
    if (parser_types[i] == value) return i;
  parser_types.push_back(value);
  return parser_types.size()-1;
}
...
int yyerror(const char *s);
 ...
 namespace Parser {
  ...
  bool set_alignment(int ipack)
  {
	...
	Parser::debug.class_pack_alignment = ipack;
    return true;
  }
...
TType make_ref(TType t)
 { Type value=AsType(t); value.make_reference(); return AsTType(value); }
TType incr_ptr(TType t)
 { Type value=AsType(t); value.incr_pointer(); return AsTType(value); }
TType make_unsigned(TType t)
 { Type value=AsType(t); value.make_unsigned(); return AsTType(value); }
...
void set_function_code(bool is_fn) //FIXME: throws exception if return is int and return type is void
{
...
void ParserState::reset()
{
  ...
  extern_flag_C = false;
  modifier = None;
  enum_value = enum_seen = 0;
  active_enum = NULL;
  enum_redeclaration = false;
  err = "";
  arg_list.clear();
...
// *ch 1.2.9 patch
     ...
     Template::grab_function_body(plain_method);
    ...
    // make sure they're declared
     ...
     in_template = ti;
     string header = Template::generate_function_header(pf,plain_method,false);
     string body = Template::grab_function_body(plain_method);
     #ifdef _FAKE_IOSTREAM
      header += body;
     #else
	  header.append(body);
     #endif
     pf->class_context()->add_defered_method_body(pf,header);
...
bool do_case(PExpr e)
{
 if (e) {
   int val;
   try { val = const_int_expr(e); }
   catch(string msg) { error(msg); return false; }
...
////////// implementing goto
static Label* create_goto_label(const char* label_name)
{
// NB to create the label in the overall function context, not some local block
 ...
 label_entry->data = vm_from_ptr(label);
 return label;
}
...
void do_goto(const char* label_name)
{
 try {
  Label* label = NULL;
...
void goto_label_new(const char* name)
{
...
// also attaches the list of parameter names to the signature.
 ...
 StringList sl;
 if(arg_list.size() > 0) { //FIX 1.3.0: functions with no argument do not crash uc anymore
 ...
 for(ali = arg_list.begin(); ali != arg_list.end();  ++ali) {
    ...
    sz += (ts.back().is_double() ? sizeof(double) : sizeof(VMWord));
 }
 Table *pc = &context();
...
string sig_as_str(Signature* sig, const string& name)
{
  ...
  return buff;
    ...
    ostringstream out; //!
    out << *sig;
    return buff; //WARNING: This returns buffer?!?
 #endif
}
...
Function *ParserState::start_function(Type t, const string& name,bool init_context, int ftype)
{
 ...
 try {
  ...
  fn->builtin(ftype);
  if (fn->builtin() && !fn->import_scheme()) fn->import_scheme(Import::scheme());
...
// The trick here is to explicitly load this module's anonymous namespace
  ...
  if (mode != ARG) { // i.e. default!
    ...
    if (init && init->is_const()) {
        ...
        int unit_size = t.size();
        if (bit_size < 0 || bit_size > 8*unit_size)
            fail("Bit field width exceeds its declared base type");
        if (bit_size == 0) {
            if (name != "") fail("A zero-width bit field must be unnamed");
            bit_offs = 0;
            field_size = 0;
            init = NULL;
            pe->set_bit_field(alloc_context->size(),0,0);
        } else {
        if (bit_offs && bit_offs + bit_size > field_size) bit_offs = 0;
        if (bit_offs == 0) {
            offset = alloc_context->alloc_aligned(unit_size,t.alignment(),NULL);
            field_size = 8*unit_size;
        }
        pe->set_bit_field(offset,bit_offs,bit_size);
        bit_offs += bit_size;
        if (bit_offs == field_size) bit_offs = 0;
        init = NULL;
        }
    } else {
       pe->data = is_class_context
         ? alloc_context->alloc_aligned(size,static_cast<int>(t.alignment()),NULL)
         : alloc_context->alloc(size,NULL);
	   bit_offs = 0;
    }
    ...
    if (is_local(alloc_context)) pe->data = -pe->data - vm_word_count(size);
    if(s_twodim_array > 1) { // two-dimensional array
    ...
    if (t.is_class()) {
     if(t.is_object() && pc->get_VMT() != NULL) {    // i.e first dword is for the VMT entry
       (pe->data) += (pe->is_stack_relative() ? 1 : sizeof(VMWord));
     }
    }
  } else
  if (is_local(&context())) {
...
rest_of_init:
  ...
  if (init || !is_class_context) {
   ...
   if (init || (t.is_object() && mode != DEFER_TEMP/* && !in_typedef*/)) {
    if (init && (!alloc_context || is_class_context))
...
// *fix 1.2.1 Watch out for unsigned shorts...
  ...
  if (t.is_bool()) return *(bool *)ptr ? 1 : 0;
...
static PEntry add_named_integer_constant(ParserState& parser, Type type,
                                         const string& name, int value)
{
  parser.check_context(Immediate);
  if (parser.context().lookup(name,false) != NULL)
    fail(name + " already defined");

  type.make_const();
  PEntry entry = parser.context().add(name);
  entry->type = type;
  entry->size = 1;
  entry->rmode = DIRECT;
  entry->m_typename = false;
  entry->data = global().alloc(type.size(),NULL);

  void *storage = entry->global_ptr();
  if (type.is_bool())
    *static_cast<bool *>(storage) = value != 0;
  else if (type.is_enum() || type.is_int())
    *static_cast<VMInt *>(storage) = static_cast<VMInt>(value);
  else
    fail("unsupported named integer constant type");
  return entry;
}
...
try {
  ...
  if (!debug.c_mode && pe && !nameless && !pe->type.is_enum())
  ...
  if (! pe) {
     ...
     pe->type = et;
      ...
      enum_obj = pe->type.as_enum();
      enum_redeclaration = !nameless;
  }
   active_enum = enum_obj;
...
void ParserState::add_enum(Type et, string name, PExpr init)
{
  ...
  try {
   if (init)
      enum_value = const_int_expr(init);
   PEntry existing = context().lookup(name,false);
   if (enum_redeclaration) {
     PEntry original=active_enum->find_entry(name);
     if (original==NULL || existing!=original)
       fail("incompatible enum redeclaration");
     if (*(int *)original->global_ptr()!=enum_value)
       fail("changed enum value in redeclaration");
     ++enum_value; ++enum_seen; return;
   }
   if (existing != NULL) fail("enumerator already defined");
   entry = add_named_integer_constant(*this,et,name,enum_value++);
   active_enum->add_entry(entry);
   ++enum_seen;
  } catch(string msg)
  { error(msg); }
...
void ParserState::finish_enum(bool has_definition)
{
 if (has_definition && enum_redeclaration && active_enum &&
     enum_seen!=active_enum->entry_count())
   error("incompatible enum redeclaration");
 active_enum=NULL;
 enum_redeclaration=false;
 enum_seen=0;
}
...
void do_start_try_block()
{
 ...
 // the marker object carries the handler address, so the initializer has to be
 // pointer-typed; an int constant truncates it on LLP64 hosts
 PEntry pe = state.add_variable(tbc,"_tbo_",
	                          Expressions::constant_op(t_void_ptr,vm_from_ptr(pch)));
 ...
 if ( (sig->size() != 1 && !catch_all) || (sig->size() != 0 && catch_all))
  ...
  if (!catch_all) {
    ...
    if (t.is_class() && !t.is_pointer()) {
        ...
        // _xo_ is void*, so a scalar catch parameter needs the cast back
        eco = typecast_op(0,t,eco);
...
default_case:
   ...
   if (t != t_void) {
       VMWord tinfo; int op;
       if (e->is_variable()) { op = SHOWV; tinfo = vm_from_ptr(e->entry()); }
       else  {  op = SHOWI;  tinfo = AsTType(t); }
       code().emit(op,DIRECT,global().alloc(sizeof(VMWord),&tinfo));
   }
...
Namespace* lookup_namespace(const char *name)
{
...
PEntry add_std_exception(const char *name)
{
  PEntry pe = state.context().lookup(name);
...
PExpr get_entry_expr(const char *name)
{
 PEntry pe = state.context().lookup(name);
...
void init()
{
 ...
 UCContext::init();
 Builtin::init();
 global().alloc(sizeof(VMWord),NULL); // skip first word of DIRECT address space
 // throw_exception() stores the thrown object's address here, so the slot has
 // to be pointer-wide; an int slot both truncates and overruns on LLP64 hosts
 mExceptObj = state.add_variable(t_void_ptr,"_xo_",NULL,None);
// *add 1.2.6 this points to two words used for object return-by-value, if required.
 ...
 // Language literals are direct constants, not compatibility variables with
 // separately allocated initializer entries.
 add_named_integer_constant(state,t_bool,"true",1);
 add_named_integer_constant(state,t_bool,"false",0);

 // UnderC's C++98 compatibility model uses the Windows-width unsigned short
 // representation for wchar_t.  It is a typedef rather than a distinct native
 // type in the historical type system.
 state.add_typedef(t_ushort,"wchar_t");
