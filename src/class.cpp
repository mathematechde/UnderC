...
namespace {// private utility stuff
  ...
  PEntry begin_method(string name, Type rtype, const char *arg_name, Type atype, int ftype);
  void declare_method(string name, Type rtype, const char *arg_name, Type atype, int ftype);
  void   end_method();
  ...
  const char *TRY_BLOCK_HANDLER_CLASS = "_tbhc_";
};
...
void Class::set_base_class(Class *base, int access)
{
  ...
  set_pack_alignment(Parser::debug.class_pack_alignment);

  if (base) {
        PPClass vmt;
        m_VMT = NULL;  // hm, really shd clear out if NOT null...
        // *NB* use the actual size, not the VMT-adjusted size!
        m_data = reinterpret_cast<char *>(static_cast<uintptr_t>(m_parent->size()));
...
void Class::add_defered_method_body(Function* fn, string body)
{
  m_DeferedBodies.push_back(new MethodBody(fn,body));
}
...
void Class::compile_methods()
{
 if (Parser::debug.skip_method_bodies) {
      ...
      while (m_DeferedBodies.size() > 0) {
		  ...
		  //cout << "! " << s.c_str() << endl << "* " << s << endl << "_ " << mb->body.c_str() << endl;
          line = pm->get_function_line(mb->pf);
          try {
            success = uc_eval(buff,false,true,file.c_str(),line) == 0;
          } catch(string msg) {
            success = false;
          }
          if (! success) {
...
//...add a word to class size if we have a VMT!!
    VMByteSize sz = Allocator::size();
    sz += ((m_VMT) ? sizeof(VMWord) : 0) + ((m_vtable_size) ? sizeof(VMWord) : 0);
    VMByteSize alignment = aggregate_alignment();
    if (alignment > 1 && sz % alignment != 0)
      sz += alignment - sz % alignment;
    if (sz > static_cast<VMByteSize>(INT_MAX)) {
      error("class size exceeds the public int size limit");
      return INT_MAX;
    }
...
void Class::set_imported()
{
 if (! m_import) {
   ...
   if (! base_class() && ! m_import->vtable_at_begining())
       m_import->set_first_virtual_class(Allocator::size()/sizeof(VMWord));
 }
}
...
// This implements VTABLE_PATCH, which is found in the ctors of imported classes w/ vtables
	...
	if (m_import->patched())
     ...
     // and patch any slots that we have overrided....
      std::list<int>::iterator ili = m_vtable_slots.begin();
...
PClass Class::generate_try_block_handler()
{
 using namespace Parser;
 using namespace Expressions;
 // Construct a class with a single pointer-wide field and a constructor
 // for initializing same...NB that it has a VMT so that it gets put
 // on the ODS, where we will use it as a unique marker.  The field carries a
 // CatchHandler address, so it must be pointer-wide rather than int-wide:
 // throw_exception() reads it back as a whole VM word.
  ...
  PEntry field = state.add_variable(t_void_ptr,"_field_",NULL,None);

  PEntry arg = begin_method(pc->constructor_name(),t_void,"_arg_",t_void_ptr,IsConstructor);
...
PEntry Enum::find_entry(const string& name)
{
...
PEntry begin_method(string name, Type rtype, const char *arg_name, Type atype, int ftype)
{
 ...
 if (arg_name) {
	return pfn->context()->lookup(arg_name);
 } else
    return NULL;
}
