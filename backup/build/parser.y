...
%token IF ELSE WHILE DO FOR SWITCH CASE RETURN CONTINUE BREAK DEFAULT
%token NAMESPACE USING TRY CATCH THROW TEMPLATE EXTERN
%token THREEDOT TYPEOF EXPLICIT FRIEND LAMBDA FAKE_INIT_LIST
%token <val> CONST STATIC STDCALL API VIRTUAL PRIVATE PROTECTED PUBLIC
...
%type <val> poss_unsigned poss_int poss_const poss_enum_list typecast_type
...
|  FRIEND CLASS class_name ';'{ state.add_friend_class($3); } 
;
...
enum_stmt:
  ...
  poss_enum_list
  { state.finish_enum($5 != 0); dcl_reset(); }
;

poss_enum_list: /*empty*/ { $$=0; }
| '{' enum_list '}' { $$=1; }
;
...
| _expr BIN_OR _expr { $$=bin_op(BIN_OR,$1,$3); }
